    ...
    class Program
    {
        public static void Main()
        {
            ...
            case 20:
                ...
                {
                    object NaN = null;
                    Console.WriteLine(Formula.func((int)NaN));          // 1

                        Formula.cards((int)NaN, (int)NaN);              // 2

                        numSum.numsCase5((int)NaN, (int)NaN, (int)NaN); // 3

                        Formula.funcCase((int)NaN, (int)NaN, (int)NaN); // 4
                } catch (NullReferenceException e) { Console.WriteLine($"Произошла ошибка | {e.Message}"); }
                break;
            }
        }
    }
